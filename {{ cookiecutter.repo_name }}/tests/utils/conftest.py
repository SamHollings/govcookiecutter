...
def create_dummy_text_file(
    tmp_path: Path, test_input_filename: str, test_input_text: str
) -> Path:
...
@pytest.fixture
def create_dummy_text_files(
    tmp_path: Path, test_input_filenames: List[str], test_input_texts: List[str]
) -> List[Path]:
    """Create dummy files with text in them."""
    test_input_filepaths = []
    for test_input_filename, test_input_text in zip(
...
def patch__read_and_clean_file(mocker) -> MagicMock:
    ...
    return mocker.patch(
        "src.utils.dotenvrc_to_dotenv._read_and_clean_file",
        side_effect=_read_and_clean_file,
    )
...
def patch__get_source_env_file_path(mocker) -> MagicMock:
    ...
    return mocker.patch(
        "src.utils.dotenvrc_to_dotenv._get_source_env_file_path",
        side_effect=_get_source_env_file_path,
    )
...
@pytest.fixture
def load_dotenvrc_patches(
    patch__read_and_clean_file: MagicMock,
    patch__get_source_env_file_path: MagicMock,
    patch__replace_text: MagicMock,
    patch__extract_environment_variables_from_export_attributes: MagicMock,
    patch__identical_to_existing_env_file: MagicMock,
    patch__write_file: MagicMock,
    patch_load_dotenv: MagicMock,
) -> Dict[str, MagicMock]:
    """Compile the patches for the ``load_dotenvrc`` function."""
    return {
        "_read_and_clean_file": patch__read_and_clean_file,
        "_get_source_env_file_path": patch__get_source_env_file_path,
        "_replace_text": patch__replace_text,
        "_extract_environment_variables_from_export_attributes": patch__extract_environment_variables_from_export_attributes,  # noqa: E501
        "_identical_to_existing_env_file": patch__identical_to_existing_env_file,
        "_write_file": patch__write_file,
