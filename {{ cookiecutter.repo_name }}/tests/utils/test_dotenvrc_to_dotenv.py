...
@pytest.mark.parametrize(
    "test_input_filename, test_input_text", args__read_and_clean_file
)
def test__read_and_clean_file(
    create_dummy_text_file: str, test_input_text: str
) -> None:
...
args__replace_text = [
    ...
    (
        ...
        {"some": "other", "more": "additional"},
        "Some text, other additional text",
    ),
    (
        "Swap this and that with that and this",
        {"this": "that", "that": "this"},
...
@pytest.mark.parametrize(
    "test_input_text, test_input_replacements, test_expected", args__replace_text
)
def test__replace_text(
    test_input_text: str, test_input_replacements: Dict[str, str], test_expected: str
) -> None:
...
def test__get_source_env_file_path(
    test_input: str, test_expected: Optional[str]
) -> None:
...
# Define test cases for the
# `test__extract_environment_variables_from_export_attributes` test
...
def test__extract_environment_variables_from_export_attributes(
    test_input, test_expected
) -> None:
    """Test the ``_extract_environment_variables_from_export_attributes`` function
    returns as expected."""
    if test_expected:
        assert (
            _extract_environment_variables_from_export_attributes(test_input)
            == test_expected
        )
    else:
...
class TestIdenticalToExistingEnvFile:
    @pytest.mark.parametrize(
        "test_input_env_variables", [a[2] for a in args__identical_to_existing_env_file]
    )
    def test_returns_correctly_for_no_env_file(
        self, test_input_env_variables: List[str]
    ) -> None:
        """Test the ``_identical_to_existing_env_file`` function returns as expected if
        the file does not exist."""
        assert not _identical_to_existing_env_file(
            "non_existent.file", test_input_env_variables
        )

    @pytest.mark.parametrize(
        "test_input_filename, test_input_text, test_input_env_variables, test_expected",
        args__identical_to_existing_env_file,
    )
    def test_returns_correctly_for_env_file(
        self,
        create_dummy_text_file: str,
        test_input_env_variables: List[str],
        test_expected: bool,
    ) -> None:
        """Test the ``_identical_to_existing_env_file`` function returns as expected if
        the file exists."""
        assert (
            _identical_to_existing_env_file(
                create_dummy_text_file, test_input_env_variables
            )
            == test_expected
        )
...
@pytest.mark.parametrize(
    "test_input_strings, test_input_filename, test_expected", args__write_file
)
def test__write_file(
    tmpdir, test_input_strings: List[str], test_input_filename: str, test_expected: str
) -> None:
    """Test the ``_write_file`` function returns as expected."""

    # Create a temporary file, and write out the data, then parse it and assert it is
    # as expected
...
args_integration__get_source_env_file_path = [
    ...
    (
        ".file2",
        'export A=1\nsource_env "b"\nexport C=3',
        ["export A=1", 'source_env "b"', "export C=3"],
    ),
    (
        ".file2",
        "export A=1\nsource_env 'b'\nexport C=3",
...
args_integration__replace_text__replacements = [
    None,
    {"a": 1, "b": 2},
    {"c": 3, "d": 4},
]
...
args_integration__write_file__env = (
    args_integration__identical_to_existing_env_file__env
)
...
args_integration_load_dotenv__env = (
    args_integration__identical_to_existing_env_file__env
)
...
class TestIntegrationLoadDotenvrc:
    def test__read_and_clean_file_uses_default_envrc(
        self, load_dotenvrc_patches: Dict[str, MagicMock]
    ) -> None:
        """Test the ``_read_and_clean_file`` function is called correctly if
        ``envrc=None``."""

        # Execute the `load_dotenvrc` and assert `_read_and_clean_file` is called with
        # both the `.envrc` and `.secrets` file paths
        load_dotenvrc(envrc=None)
        load_dotenvrc_patches["_read_and_clean_file"].assert_has_calls(
            [
                call(DIR_REPOSITORY.joinpath(".envrc")),
                call(DIR_REPOSITORY.joinpath(".secrets")),
            ]
        )

    @pytest.mark.parametrize(
        "test_input_filename, test_input_text, test_expected",
        args_integration__read_and_clean_file,
    )
    def test__read_and_clean_file_called_correctly(
        self,
        load_dotenvrc_patches: Dict[str, MagicMock],
        create_dummy_text_file: str,
        test_expected: List[str],
    ) -> None:
        """Test the ``_read_and_clean_file`` function is called correctly."""

        # Define the file path to the ".envrc" file, and then any subsequent sourced
        # files, if relevant
        test_input_file = Path(create_dummy_text_file)
        test_expected_files = [
            call(test_input_file),
            *[call(test_input_file.parent.joinpath(f)) for f in test_expected[1:]],
        ]

        # Set the side effect of the `_read_and_clean_file` patch. Use `DEFAULT` to
        # restore normal MagicMock side effects after the first call of
        # `_read_and_clean_file` - must be the same as number of times we expect
        # `_read_and_clean_file` to be called (minus the first call), otherwise a
        # `StopIteration` error is returned
        load_dotenvrc_patches["_read_and_clean_file"].side_effect = [
            _read_and_clean_file(test_input_file),
            *[DEFAULT] * len(test_expected[1:]),
        ]

        # Execute the `load_dotenvrc` function, and assert it is called with the
        # correct arguments in the correct order
    ...
    def test__get_source_env_file_path_called_correctly(
        self,
        load_dotenvrc_patches: Dict[str, MagicMock],
        create_dummy_text_file: str,
        test_expected: List[str],
        ...
        # `StopIteration` error is returned
        load_dotenvrc_patches["_read_and_clean_file"].side_effect = [
            _read_and_clean_file(create_dummy_text_file),
            *[DEFAULT] * len(test_expected[1:]),
        ]
        ...
        load_dotenvrc_patches["_get_source_env_file_path"].assert_has_calls(
            [call(e) for e in test_expected]
        )

    @pytest.mark.parametrize(
        "test_input_filenames, test_input_texts, test_expected",
        args_integration__replace_text,
    )
    @pytest.mark.parametrize(
        "test_input_replacements", args_integration__replace_text__replacements
    )
    def test__replace_text_called_correctly(
        self,
        load_dotenvrc_patches: Dict[str, MagicMock],
        ...
        # Execute the `load_dotenvrc` using the first file in `create_dummy_text_files`
        # as `.envrc`
        load_dotenvrc(
            envrc=create_dummy_text_files[0], replacements=test_input_replacements
        )

        # Assert the calls are as expected
        test_input_replacements = (
            test_input_replacements if test_input_replacements else REPLACEMENTS
        )
    ...
    @pytest.mark.parametrize(
        "test_input_filenames, test_input_texts, test_expected_call_count",
        args_integration__extract_environment_variables_from_export_attributes,
    )
    def test__extract_environment_variables_from_export_attributes_called_correctly(
        ...
        # Assert the calls are as expected
        load_dotenvrc_patches[
            "_extract_environment_variables_from_export_attributes"
        ].assert_has_calls(
            [call(load_dotenvrc_patches["_replace_text"].return_value)]
    ...
    @pytest.mark.parametrize(
        "test_input_env", args_integration__identical_to_existing_env_file__env
    )
    def test__identical_to_existing_env_file_called_correctly(
        self,
        load_dotenvrc_patches: Dict[str, MagicMock],
        create_dummy_text_files: List[Path],
    ...
    def test__write_file_not_called_correctly(
        self, load_dotenvrc_patches: Dict[str, MagicMock]
    ):

        # Set the return value of the `_identical_to_existing_env_file` patch to be
        # False
    ...
    def test__write_file_called_correctly(
        self,
        load_dotenvrc_patches: Dict[str, MagicMock],
        create_dummy_text_files: List[Path],
        test_input_env: Optional[str],
    ...
    @pytest.mark.parametrize(
        "test_input_filenames, test_input_texts", args_integration_load_dotenv
    )
    ...
    def test_load_dotenv_called_correctly(
        ...
        create_dummy_text_files: List[Path],
        test_input_env: Optional[str],
        test_input_load_env: bool,
        test_input_override_existing_env: bool,
    ) -> None:
        ...
        # as `.envrc`
        load_dotenvrc(
            envrc=create_dummy_text_files[0],
            env=test_input_env,
            load_env=test_input_load_env,
    ...
    @pytest.mark.parametrize(
        "test_input_filenames, test_input_texts",
        args_integration_load_dotenv_after__write_file,
    )
    def test_load_dotenv_after__write_file(
        self,
        mocker,
        load_dotenvrc_patches: Dict[str, MagicMock],
        create_dummy_text_files: List[Path],
        ...
        """Test the ``load_dotenv`` function is called after the ``__write_file``
        function."""
        ...
        # Set the return value of the `_identical_to_existing_env_file` patch to be
        # False, and set the side effect of the `_write_file` patch
        load_dotenvrc_patches["_identical_to_existing_env_file"].return_value = False
        ...
        # Execute the `load_dotenvrc` using the first file in `create_dummy_text_files`
        # as `.envrc`, and the second file as the `.env` file
...
@pytest.mark.parametrize(
    "test_input_filenames, test_input_texts, test_input_replacements, test_expected",
    args_load_dotenvrc_returns_correctly,
)
@pytest.mark.parametrize("test_input_override_existing_env", [True, False])
def test_load_dotenvrc_returns_correctly(
    capfd,
    patch_load_dotenv: MagicMock,
    create_dummy_text_files: List[Path],
    test_input_replacements: Dict[str, str],
    test_input_override_existing_env: bool,
    test_expected: Dict[str, Any],
) -> None:
    ...
    # Execute the `load_dotenvrc` function, and assert the correct environment
    # variables are printed; assume the first file is the `.env` file, and the second
    # is the `.envrc` file
    load_dotenvrc(
        create_dummy_text_files[1],
        test_input_replacements,
        create_dummy_text_files[0],
        override_existing_env=test_input_override_existing_env,
    )
